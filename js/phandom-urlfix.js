// Get the current link 
        ...
        // We need variables that can store lists of Google Searches
        var primarySearchObject = [];
        var sideSearchObject = [];
        var userSearchObject1 = [];
        var userSearchObject2 = [];


        function customLink(){
            //get headline
            var custom_headline = $('.custom-headline').val().capAll();
            var custom_images = [];
            var t = []
            //get custom-image values & create URLs
            $('.nav-tabs .active a').each(function(){
                var id = $(this).attr('href');
                var value = $(id+' .custom-image').val();
                if (id === '#google1' || id === '#google2'){
                    custom_images.push(value);
                } else if (id === '#facebook1' || id === '#facebook2'){
                    custom_images.push('https://graph.facebook.com/'+value+'/picture?type=large');
                } else if (id === '#twitter1' || id === '#twitter2'){
                    custom_images.push('http://api.twitter.com/1/users/profile_image?screen_name='+value+'&size=original');
                    t.push('@'+value);
                }
            });

            var h = custom_headline ? custom_headline.replace(/\s/g,"+") : '';
            var u = custom_images;

            var new_link = basePath+"?"+"h="+h.encode()+"&u="+u.encode()+"&t="+t.encode();
            if (h != '' && i != ''){
        ...
        function getShareUrl(){
            ...
            if (params) {
                ...
                h = headline_object.headline.replace(/\s/g,"+").encode();
                g = headline_object.google_searches.encode();
                t = headline_object.celebs_twitter.encode();
                shareUrl = basePath+"?"+"h="+h+"&g="+g+"&t="+t;
            };
        ...
        $(document).ready(function(){
            ...
            function userSearchHandler(query,searchObject,toDestination, target){
                searchImages([query],searchObject,toDestination,target);
            }

            $('#google-search-1').click(function(){
                var query = $('#google-search-1-input').val();
                userSearchObject1 = [];
                userSearchHandler(query,userSearchObject1,toUserSearch,'#google-search-1-results');
            });

            $('#google-search-2').click(function(){
                var query = $('#google-search-2-input').val();
                userSearchObject2 = [];
                userSearchHandler(query,userSearchObject2,toUserSearch, '#google-search-2-results');
            });
...
//                    return false;
            ...
            function getParams(){
                ...
                var params = escapedPath.split('#')[1];
                if (params){
                    params = params.replace(/&t$/,'&t=');
                    var parseString = '{"' + 
                        params.replace(/&/g, "\",\"")
                        .replace(/=/g,"\":\"")
                        .decode()
                        .replace(/\+/g,' ') + '"}';
                    params = JSON.parse(parseString);
                    params.u = params.u ? params.u.split(',') : '';
                    params.g = params.g ? params.g.split(',') : '';
                    params.t = params.t ? params.t.split(',') : '';
                    headline_object.headline = params.h;
                    headline_object.urls = params.u;
                    headline_object.google_searches = params.g;
            ...
            function makeParams(obj){
                var h = escape(obj.headline.replace(/\s/g,'+'));
                var g = escape(obj.google_searches);
                var t = escape(obj.celebs_twitter);
                var new_link = basePath+"?"+"h="+h+"&g="+g+"&t="+t;
                return new_link;
            }
            ...
            function toHeadline(number, searchObject){
                ...
                if (searchObject[number].results && searchObject[number].results.length > 0) {
                    ...
                    newImg.attr({'src':result.unescapedUrl})
                        ...
                        .load(function(){
                            ...
                            if (pictures_loaded === primarySearchObject.length) {
                                ...
                                if (google_searches.length === 1 && this.height > this.width){
                        ...
                        .error(function(){
                            searchImages([google_searches[number]],primarySearchObject,toHeadline);
                        });
                }
            }

            function toSide(number, searchObject,target){
                if (searchObject[number].results && searchObject[number].results.length > 0) {
                    var results = searchObject[0].results;
                    var result = results.getRandom();
                    var src = result.unescapedUrl;
                    $('<img>').attr({
                        'src':src
                    })
                    .css({'width':'335px'})
            ...
            function toUserSearch(number,searchObject,target){
                $(target).empty();
                $(target).siblings('.custom-image').val('');
                if (searchObject[number].results && searchObject[number].results.length > 0) {
                    var results = searchObject[0].results;
                    for (var i = 0; i < results.length; i++) {
                        var result = results[i];
                        var tb = result.tbUrl;
                        var big = result.unescapedUrl;
                        $('<img>').attr({
                            'src':tb,
                            'data-bigUrl':big
                            })
                            .addClass('google-search-image')
                            .click(function(){
                                unselectAll(target);
                                $(this).addClass('google-search-image-selected');
                                var big_url = $(this).attr('data-bigUrl');
                                $(this).parent().siblings('.custom-image').val(big_url);
                            })
                            .load(function(){
            ...
            function unselectAll(target){
                $(target+" img").each(function(){
                    $(this).removeClass('google-search-image-selected');
                });
            }

            function select(img){
                $(img).addClass('google-search-image-selected');
            }

            // Find medium size pics of each celeb's face
            // 'toDestination' is a function, the result handler
            function searchImages(query, searchObject, toDestination, target){
                ...
                for (var i = 0; i < query.length; i++) {
                    ...
                    if (twitter_handle !== null) {
                        ...
                        searchObject[i].setResultSetSize(8);
                        ...
                        searchObject[i].setSearchCompleteCallback(this, toDestination, [i, searchObject, target]);
                        searchObject[i].execute(query[i]+' -imdb');                        
                    }
                };
            }

            //create next headlines
            for (var i = 0; i < 2; i++) {

                var next_headline_object = {};
                ...
                var hook_text_array = ["crazy","sexy","exclusive","not dead","werewolf?","secrets","lawsuit","co-star crush","accusations","fat","photoshoot","scandal","i was wrong","jailtime"];
                var hook_text = $('<span>').html(hook_text_array.getRandom());

                var truncated = next_headline_object.headline.substring(0,36);
                truncated += '...'+'&raquo;';
                var link = makeParams(next_headline_object);
                ...
                $('.next-headlines').append(text.append(container.append(imgContainer.append(hook.append(hook_text)))));
                //$('.next-headlines').append(text);
                sideSearchObject[i] = {};
                var query = next_headline_object.google_searches[0]
                searchImages([query],sideSearchObject[i], toSide,'#next'+i)
            };
            ...
            var urls = headline_object.urls;
            var google_searches = headline_object.google_searches;
            ...
            if (google_searches) {
                searchImages(google_searches,primarySearchObject, toHeadline)
            };

            if (urls) {
                for (var i = 0; i < urls.length; i++) {
                    var link = urls[i];
                    primarySearchObject[i] = {results:[{unescapedUrl:link}]};
                    toHeadline(i,primarySearchObject);
                };
            };
...
String.prototype.celebrify = function(celebs,words){
    ...
    headline_object.google_searches = chosen_celebs_arrays;
